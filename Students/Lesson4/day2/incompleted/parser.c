...
void compileProgram(void) {
  ...
  prog = createProgramObject(currentToken->string);
...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      ...
      eat(SB_EQ);
      cval = compileConstant();
      obj->constAttrs->value = cval;
      declareObject(obj);
      eat(SB_SEMICOLON);
...
void compileBlock2(void) {
  ...
  if (lookAhead->tokenType == KW_TYPE) {
    ...
    do {
      ...
      obj->typeAttrs->actualType = actualtype;
      declareObject(obj);
      eat(SB_SEMICOLON);
    } while (lookAhead->tokenType == TK_IDENT);
...
void compileFuncDecl(void) {
  ...
  compileParams();
  eat(SB_COLON);
  Rtype = compileBasicType();
  obj->funcAttrs->returnType = Rtype;
...
Type* compileType(void) {
  ...
  case KW_ARRAY:
    ...
    eat(KW_OF);
    eleType = compileType();
    type = makeArrayType(size, eleType);
    break;
  case TK_IDENT:
    eat(TK_IDENT);
    Object* obj = lookupObject(currentToken->string);
